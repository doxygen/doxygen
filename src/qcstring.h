...
inline int qstrcmp(const char *str1, const char *str2)
{
  return (str1 && str2) ? strcmp(str1, str2) : // both non-empty
           (qisempty(str1) && qisempty(str2)) ? 0
                                              : // both empty
           qisempty(str1) ? -1
                          : 1; // one empty, other non-empty
}
...
class QCString
{
  public:
    ...
    QCString(const char *str, size_t maxlen) :
        m_rep(str ? str : "") { m_rep.resize(maxlen); }
    ...
    {
      return m_rep.empty() ? QCString() : len < m_rep.size() ? QCString(m_rep.substr(m_rep.size() - len, len))
                                                             : *this;
    }
    ...
    QCString mid(size_t index, size_t len = static_cast<size_t>(-1)) const
    {
      ...
      return m_rep.empty() || index > slen || len == 0 ? QCString() : QCString(m_rep.substr(index, len));
    }
