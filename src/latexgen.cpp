...
LatexGenerator::LatexGenerator() :
    OutputGenerator(Config_getString(LATEX_OUTPUT)), m_codeList(std::make_unique<OutputCodeList>())
{
...
void LatexGenerator::endIndexSection(IndexSection is)
{
  ...
  case IndexSection::isPageDocumentation:
  {
    ...
    {
      if (!pd->getGroupDef() && !pd->isReference() && !pd->hasParentPage() && pd->name() != "citelist" && Doxygen::mainPage.get() != pd.get())
      {
