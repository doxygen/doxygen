...
class GrowBuf
{
  public:
    GrowBuf() :
        m_str(nullptr), m_pos(0), m_len(0) {}
    GrowBuf(size_t initialSize) :
        m_pos(0), m_len(initialSize) { m_str = static_cast<char *>(malloc(m_len)); }
    ~GrowBuf() { free(m_str); }
    ...
    GrowBuf(GrowBuf &&other) :
        m_str(std::exchange(other.m_str, static_cast<char *>(nullptr))), m_pos(std::exchange(other.m_pos, 0)), m_len(std::exchange(other.m_len, 0))
    {
    }
    ...
    void reserve(size_t size)
    {
      ...
      {
        m_len = size;
        m_str = static_cast<char *>(realloc(m_str, m_len));
      }
    }
    void clear() { m_pos = 0; }
    void addChar(char c)
