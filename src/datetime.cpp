...
QCString formatDateTime(const QCString &format, const std::tm &dt, int &formatUsed)
{
  ...
  while ((c = *p++))
  {
    ...
    case '%':
      ...
      case 'y':
        growBuf.addInt(fmt_selected, getYear(dt) % 100);
        formatUsed |= SF_Date;
        break;
      case 'Y':
        growBuf.addInt("%d", getYear(dt));
        formatUsed |= SF_Date;
        break;
      case 'm':
        growBuf.addInt(fmt_selected, getMonth(dt));
        formatUsed |= SF_Date;
        break;
      case 'b':
        growBuf.addStr(theTranslator->trMonth(getMonth(dt), false, false));
        formatUsed |= SF_Date;
        break;
      case 'B':
        growBuf.addStr(theTranslator->trMonth(getMonth(dt), false, true));
        formatUsed |= SF_Date;
        break;
      case 'd':
        growBuf.addInt(fmt_selected, getDay(dt));
        formatUsed |= SF_Date;
        break;
      case 'u':
        growBuf.addInt("%d", getDayOfWeek(dt)); /* Monday = 1 ... Sunday = 7 */
        formatUsed |= SF_Date;
        break;
      case 'w':
        growBuf.addInt("%d", getDayOfWeek(dt) % 7); /* Sunday = 0 ... Saturday = 6 */
        formatUsed |= SF_Date;
        break;
      case 'a':
        growBuf.addStr(theTranslator->trDayOfWeek(getDayOfWeek(dt), false, false));
        formatUsed |= SF_Date;
        break;
      case 'A':
        growBuf.addStr(theTranslator->trDayOfWeek(getDayOfWeek(dt), false, true));
        formatUsed |= SF_Date;
        break;
      case 'H':
        growBuf.addInt(fmt_selected, dt.tm_hour);
        formatUsed |= SF_Time;
        break;
      case 'I':
        growBuf.addInt(fmt_selected, dt.tm_hour % 12);
        formatUsed |= SF_Time;
        break;
      case 'p':
        growBuf.addStr(theTranslator->trDayPeriod(dt.tm_hour >= 12));
        formatUsed |= SF_Time;
        break;
      case 'M':
        growBuf.addInt(fmt_selected, dt.tm_min);
        formatUsed |= SF_Time;
        break;
      case 'S':
        growBuf.addInt(fmt_selected, dt.tm_sec);
        formatUsed |= SF_Seconds;
        break;
      default:
        growBuf.addChar(c);
