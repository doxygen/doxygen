...
size_t Markdown::Private::isSpecialCommand(std::string_view data, size_t offset)
{
  ...
  static constexpr auto endOfLine = [](std::string_view data_, size_t offset_) -> size_t {
    ...
    {
      if (c == '\\')
        lc = '\\'; // last character was a line continuation
      else if (c != ' ')
        lc = 0; // rest line continuation
      offset_++;
    }
    return offset_;
  };
...
int Markdown::Private::processNmdash(std::string_view data, size_t offset)
{
  ...
  {
    AUTO_TRACE_EXIT("result={}", 1 - count);
    return 1 - count;
  } // start HTML comment
  ...
  if (i < size && data[i] == '(') // inline link
  {
    ...
    if (data[i] == '\'' || data[i] == '"')
    {
      ...
      if (data[titleEnd] == c) // found it
      {
        ...
        {
          if (data[i] == ' ')
            i++; // remove space after the closing quote and the closing bracket
          else if (data[i] == ')')
            break; // the end bracket
          else     // illegal
          {
            return 0;
          }
...
static bool isHRuler(std::string_view data)
{
  ...
  if (i >= size)
  {
    AUTO_TRACE_EXIT("result=false: empty line");
    return false;
  } // empty line
  char c = data[i];
  if (c != '*' && c != '-' && c != '_')
