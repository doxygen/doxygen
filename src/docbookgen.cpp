...
DocbookGenerator::DocbookGenerator() :
    OutputGenerator(Config_getString(DOCBOOK_OUTPUT)), m_codeList(std::make_unique<OutputCodeList>())
{
  DB_GEN_C
...
void DocbookGenerator::endIndexSection(IndexSection is)
{
  ...
  case IndexSection::isPageDocumentation:
    ...
    {
      if (!pd->getGroupDef() && !pd->isReference() && !pd->hasParentPage() && Doxygen::mainPage.get() != pd.get())
      {
                                   ...
                                   const QCString &anchor, const QCString &text)
{
  ...
  {
    if (!f.isEmpty())
      result += "<link linkend=\"_" + stripPath(f) + "_1" + anchor + "\">";
    else
      result += "<link linkend=\"_" + anchor + "\">";
  }
  else
  {
