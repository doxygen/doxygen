...
static const StringUnorderedSet                  g_compoundKeywords = { "template class", "template struct", "class", "struct", "union", "interface", "exception" };
 ...
 */
[[maybe_unused]] static Definition *buildScopeFromQualifiedName(const QCString &name_, SrcLangExt lang, const TagInfo *tagInfo)
{
    ...
    if (!root->name.isEmpty())
    {
      ...
      else
      {
        AUTO_TRACE_ADD("Found used type '{}' in scope='{}'",
                       usingDef->name(), nd ? nd->name() : fd ? fd->name()
                                                              : QCString("<unknown>"));
      }
                          ...
                          const QCString &funcDecl, const QCString &exceptions, TypeSpecifier spec)
{
  ...
  if (mn->size() > 0)
  {
    ...
    sameClass = std::equal(mn->begin() + 1, mn->end(), mn->begin(),
                           [](const auto &md1, const auto &md2) { return md1->getClassDef()->name() == md2->getClassDef()->name(); });
  }
  if (sameClass)
  {
...
static void generateNamespaceClassDocs(const ClassLinkedRefMap &classList)
{
      ...
      if (cdm)
      {
        ...
        auto processFile = [ctx]() {
          if ((ctx->cdm->isLinkableInProject() &&
               !ctx->cdm->isImplicitTemplateInstance()) // skip external references, anonymous compounds and
  ...
  else // single threaded processing
  {
    ...
    for (const auto &cd : classList)
    {
      ...
      {
        if ((cd->isLinkableInProject() &&
             !cd->isImplicitTemplateInstance()) // skip external references, anonymous compounds and
            // template instances and nested classes
  ...
  for (const auto &fn : *Doxygen::inputNameLinkedMap)
  {
    ...
    else if (size > 1) // multiple occurrences of the same file name
    {
      ...
      std::stable_sort(fileEntries.begin(),
                       fileEntries.end(),
                       [](const FileEntry &fe1, const FileEntry &fe2) { return qstricmp_sort(fe1.path, fe2.path) < 0; });
