...
DotNode::DotNode(DotGraph *graph, const QCString &lab, const QCString &tip, const QCString &url,
    ...
    m_graph(graph),
    m_number(graph->getNextNodeNumber()), m_label(lab), m_tooltip(tip), m_url(url), m_isRoot(isRoot), m_classDef(cd)
{
}
                       ...
                       bool hasNonReachableChildren) const
{
  ...
  else
  {
    ...
    fillCol = m_url.isEmpty() ? "#E0E0E0" : (hasNonReachableChildren ? "#FFF0F0" : "white");
  }
  t << "  Node" << m_number << " [";
                         ...
                         bool            pointBack) const
{
  ...
  t << "id=\"edge" << m_graph->getNextEdgeNumber() << "_Node" << QCString().sprintf("%06d", m_number) << "_Node" << QCString().sprintf("%06d", cn->number()) << "\",";
