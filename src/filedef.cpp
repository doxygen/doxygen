...
void FileDefImpl::writeClassesToTagFile(TextStream &tagFile, const ClassLinkedRefMap &list)
{
  for (const auto &cd : list)
  {
    ...
    {
      tagFile << "    <class kind=\"" << cd->compoundTypeString() << "\">" << convertToXML(cd->name()) << "</class>\n";
    }
  }
}
...
{
  static const std::unordered_set<std::string> docExtensions = { "doc", "txt", "dox", "md", "markdown" };
