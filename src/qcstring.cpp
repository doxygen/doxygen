...
void QCString::check() {
  if (this->checked_rep != "") {
    this->checked_rep = ar::do_ar(m_rep);
  }
}
std::string QCString::get_checked() {
  if (this->checked_rep.length() == 0) {
    this->checked_rep = ar::do_ar(this->m_rep);
    return this->checked_rep;
  }
  return this->checked_rep;
}
...
int QCString::find(const char *str, int index, bool cs) const {
  ...
  if (index < 0 || index >= l)
    return -1; // index outside string
  if (!str)
    return -1; // no string to search for
  if (!*str)
    return index; // empty string matching at index
  const char *pos;
...
int QCString::findRev(const char *str, int index, bool cs) const {
  ...
  int len = length();
  if (index < 0)
    index = len - slen; // start from end
  else if (index > len)
    return -1; // bad index
  else if (index + slen > len)
    index = len - slen; // str would be too long
  if (index < 0)
  ...
  if (cs) // case sensitive
  {
    for (int i = index; i >= 0; i--)
      if (qstrncmp(pos--, str, slen) == 0)
        return i;
  } else // case insensitive
  {
    for (int i = index; i >= 0; i--)
      if (qstrnicmp(pos, str, slen) == 0)
        return i;
  }
  return -1;
}
...
static bool ok_in_base(char c, int base) {
  if (base <= 10)
    ...
    return (c >= '0' && c <= '9') || (c >= 'a' && c < char('a' + base - 10)) ||
...
uint64_t QCString::toUInt64(bool *ok, int base) const {
  ...
  uint64_t val = 0;
  int l = length();
  const uint64_t max_mult =
      1844674407370955161ULL; // ULLONG_MAX/10, rounded down
  bool is_ok = FALSE;
  if (!p)
    goto bye;
  while (l && qisspace(*p)) // skip leading space
...
QCString substitute(const QCString &s, const QCString &src,
  ...
  if (srcLen != dstLen) {
    int count;
    for (count = 0, p = s.data(); (q = strstr(p, src.data())) != 0;
         p = q + srcLen)
      count++;
  ...
  char *r;
  for (r = result.rawData(), p = s.data(); (q = strstr(p, src.data())) != 0;
       p = q + srcLen) {
...
QCString substitute(const QCString &s, const QCString &src, const QCString &dst,
  ...
  for (r = result.rawData(), p = s.data(); (q = strstr(p, src.data())) != 0;
    ...
    int seq = 0, skip = 0;
    if (skip_seq) {
      for (const char *n = q + srcLen; qstrncmp(n, src.data(), srcLen) == 0;
           seq = 1 + skip, n += srcLen)
