...
RTFGenerator::RTFGenerator() :
    OutputGenerator(Config_getString(RTF_OUTPUT)), m_codeList(std::make_unique<OutputCodeList>())
{
  ...
  case IndexSection::isTitlePageAuthor:
  {
    ...
    case TIMESTAMP_t::DATETIME:
      m_t << "{\\field\\fldedit {\\*\\fldinst CREATEDATE \\\\*MERGEFORMAT}"
             "{\\fldrslt "
          << dateToString(DateTimeType::DateTime) << " }}\\par\n";
      break;
    case TIMESTAMP_t::DATE:
      m_t << "{\\field\\fldedit {\\*\\fldinst CREATEDATE \\\\*MERGEFORMAT}"
             "{\\fldrslt "
          << dateToString(DateTimeType::Date) << " }}\\par\n";
      break;
    case TIMESTAMP_t::NO:
      break;
    }
