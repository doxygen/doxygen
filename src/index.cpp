...
static void writeMemberToIndex(const Definition *def, const MemberDef *md, bool addToIndex)
{
  ...
  QCString            name                      = namespaceMemberInFileDocs || defType == Definition::TypeModule ? md->qualifiedName() : md->name();
  ...
  if (isDir)
  {
    ...
    for (const auto &emd : enumList)
    {
      if (emd->hasDocumentation())
      {
        ...
        QCString ename            = namespaceMemberInFileDocs || defType == Definition::TypeModule ? emd->qualifiedName() : emd->name();
...
template <>
const ClassDef *get_pointer(const ClassLinkedMap::Ptr &p) { return p.get(); }
template <>
const ClassDef *get_pointer(const ClassLinkedRefMap::Ptr &p) { return p; }
  ...
  for (const auto &cdi : cl)
  {
    ...
    if (cd->getLanguage() == SrcLangExt::VHDL)
    {
      if (VhdlDocGen::convert(cd->protection()) == VhdlDocGen::PACKAGECLASS ||
          VhdlDocGen::convert(cd->protection()) == VhdlDocGen::PACKBODYCLASS) // no architecture
      {
        continue;
      }
    }
...
static const CmhlInfo *getCmhlInfo(size_t hl)
{
  ...
  static CmhlInfo cmhlInfo[] = {
    ...
    CmhlInfo("functions_func",
             fortranOpt ? theTranslator->trSubprograms() : vhdlOpt ? theTranslator->trFunctionAndProc()
  ...
  if (!Config_getBool(LATEX_HIDE_INDICES))
  {
    ...
    if (index.numHierarchyClasses() > 0)
    {
      ...
      if (lne)
      {
        ol.startIndexSection(IndexSection::isClassHierarchyIndex);
        ol.parseText(/*projPrefix+*/
                     (fortranOpt ? theTranslator->trCompoundIndexFortran() : vhdlOpt ? theTranslator->trHierarchicalIndex()
