  ...
  QCString         *pResolvedType)
{
  ...
  char    *pk = key.rawData();
  qstrcpy(pk, scope->name().data());
  *(pk + scopeNameLen - 1) = '+';
  pk += scopeNameLen;
  qstrcpy(pk, name.data());
  *(pk + nameLen - 1) = '+';
  pk += nameLen;
      ...
      if (isCodeSymbol(d->definitionType()) &&
          (!onlyLinkable ||
           ...
           (d->definitionType() == Definition::TypeFile &&
            (toFileDef(d))->generateSourceFile()) // undocumented file that has source code we can link to
           ))
      {
                                             ...
                                             bool              mayBeHidden)
{
  ...
  auto lang = scope ? scope->getLanguage() : p->fileScope() ? p->fileScope()->getLanguage()
