...
def prepCDocs(node):
	...
	if (type != 'obsolete'):
		...
		if (node.hasAttribute('depends')):
			...
			dependsNode = getNodeWithId(depends.upper())
			if (dependsNode.getAttribute('type') == 'bool'):
				doc += "<br/>%s \\ref cfg_%s \"%s\" is set to \\c YES." % (
					"This tag requires that the tag", depends.lower(), depends.upper())
			else:
				doc += "<br/>%s \\ref cfg_%s \"%s\" is set." % (
					"This tag requires that the tag", depends.lower(), depends.upper())

	docC = transformDocs(doc)
	return docC;

def getNodeWithId(id):
...
def parseOptionDoc(node, first):
	...
	if (type != 'obsolete'):
		...
		if (node.hasAttribute('depends')):
			depends = node.getAttribute('depends')
			print("")
			# search for node with name depends.upper
			dependsNode = getNodeWithId(depends.upper())
			if (dependsNode.getAttribute('type') == 'bool'):
				print("%s \\ref cfg_%s \"%s\" is set to \\c YES." % (
					"This tag requires that the tag", depends.lower(), depends.upper()))
			else:
				print("%s \\ref cfg_%s \"%s\" is set." % (
					"This tag requires that the tag", depends.lower(), depends.upper()))
		return False
...
def main():
	...
	elem = fullDoc.documentElement
	optionElements = fullDoc.getElementsByTagName("option")
	if (sys.argv[1] == "-doc"):
