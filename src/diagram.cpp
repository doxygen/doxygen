...
class DiagramItem
{
  public:
    ...
    {
      m_x = static_cast<uint32_t>(m_x + dx);
      m_y = static_cast<uint32_t>(m_y + dy);
    }
...
class TreeDiagram
{
  public:
    ...
    {
      m_rows.push_back(std::make_unique<DiagramRow>(this, l));
...
static Protection getMinProtectionLevel(const DiagramItemList &dil)
{
  ...
  if (it != dil.end())
  {
    ...
    for (++it; it != dil.end(); ++it)
    {
      ...
      if (p != result)
      {
        if (result == Protection::Protected && p == Protection::Public)
          result = p;
        else if (result == Protection::Private)
          result = p;
      }
    }
  }
...
DiagramItem::DiagramItem(DiagramItem *p, uint32_t number, const ClassDef *cd,
    ...
    m_parent(p),
    m_num(number), m_prot(pr), m_virt(vi), m_templSpec(ts), m_classDef(cd)
{
}
        ...
        else // multiple children, put them in a vertical list
        {
          if (bitmap)
          {
            x = di->parentItem()->xPos() *
                  (cellWidth + labelHorSpacing) / gridWidth +
                cellWidth / 2;
            if (doBase) // base classes
            {
              ys = image->height() -
      ...
      for (const auto &di : *dr)
      {
        ...
        if (!dil.empty())
        {
          ...
          if (first != last && !first->isInList()) /* connect with all base classes */
          {
            if (bitmap)
            {
              uint32_t xs = first->xPos() * (cellWidth + labelHorSpacing) / gridWidth + cellWidth / 2;
              uint32_t xe = last->xPos() * (cellWidth + labelHorSpacing) / gridWidth + cellWidth / 2;
              if (doBase) // base classes
              {
...
struct ClassDiagram::Private
{
    Private(const ClassDef *root) :
        base(root, true), super(root, false) {}
    TreeDiagram base;
    TreeDiagram super;
};
...
ClassDiagram::ClassDiagram(const ClassDef *root) :
    p(std::make_unique<Private>(root))
{
