...
static inline MemberVector refMapToVector(const std::unordered_map<std::string, MemberDef *> &map)
{
  ...
  std::transform(map.begin(), map.end(),                      // iterate over map
                 ...
                 [](const auto &item) { return item.second; } // extract value to add from map Key,Value pair
  );
  // and sort it
...
{
  return p->details ? p->details->line : p->brief ? p->brief->line
                                                  : 1;
}

QCString DefinitionImpl::docFile() const
{
  if (p->details && !p->details->file.isEmpty())
    return p->details->file;
  else if (p->brief && !p->brief->file.isEmpty())
    return p->brief->file;
  else
    return "<" + p->name + ">";
}
...
QCString DefinitionImpl::briefDescription(bool abbr) const
{
  ...
  return p->brief ? (abbr ? abbreviate(p->brief->doc, p->def->displayName()) : p->brief->doc) : QCString("");
}
