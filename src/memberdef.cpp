...
class MemberDefImpl : public DefinitionMixin<MemberDefMutable>
{
  public:
    ...
    void                        writeDocumentation(const MemberList *ml, int memCount, int memTotal, OutputList &ol,
                                                   ...
                                                   bool inGroup, bool showEnumValues = FALSE, bool showInline = FALSE) const override;
...
{
  const ArgumentList &defArgList = (md->isDocsForDefinition()) ? md->argumentList() : md->declArgumentList();
                         ...
                         const ArgumentList &al, const QCString &meta)
{
  ...
  if (m_type.isEmpty())
    m_decl = d->name() + m_args;
  else
    m_decl = m_type + " " + d->name() + m_args;
...
void MemberDefImpl::_computeLinkableInProject()
{
  ...
  const NamespaceDef *nspace  = getNamespaceDef();
  const FileDef      *fileDef = getFileDef();
  if (!groupDef && nspace /*&& m_related==Relationship::Member*/ && !nspace->isLinkableInProject() && (fileDef == nullptr || !fileDef->isLinkableInProject()))
  {
                                     ...
                                     bool inGroup, int indentLevel, const ClassDef *inheritedFrom, const QCString &inheritId) const
{
  ...
  OutputGenerator::MemberItemType anonType   = isAnonType ? OutputGenerator::MemberItemType::AnonymousStart : !m_tArgList.empty() ? OutputGenerator::MemberItemType::Templated
  ...
  if (!argsString().isEmpty() && !isObjCMethod())
  {
    ...
    linkifyText(TextGeneratorOLImpl(ol), // out
                ...
                getBodyDef(),            // fileScope
                this,                    // self
                isDefine() ? substitute(argsString(), ",", ", ") : isTypedef() ? substitute(argsString(), ")(", ") (")
...
bool              MemberDefImpl::hasDetailedDescription() const
{
  ...
  if (!m_hasDetailedDescriptionCached)
  {
    ...
    bool docFilter =
      ...
      (m_templateMaster ? m_templateMaster->argumentList().hasDocumentation() : m_defArgList.hasDocumentation()) ||
      (m_templateMaster ? m_templateMaster->templateArguments().hasTemplateDocumentation() : m_tArgList.hasTemplateDocumentation()) ||
...
StringVector MemberDefImpl::getLabels(const Definition *container) const
{
       ...
       TypeSpecifier(m_memSpec).setInline(false) != TypeSpecifier()))
  {
    ...
    {
      if (isFriend())
        sl.emplace_back("friend");
      else if (isRelated())
        sl.emplace_back("related");
      else
      {
        ...
        if (isNonAtomic())
          sl.emplace_back("nonatomic");
        else if (isObjCProperty())
          sl.emplace_back("atomic");

        // mutual exclusive Objective 2.0 property attributes
        if (isAssign())
          sl.emplace_back("assign");
        else if (isCopy())
          sl.emplace_back("copy");
        else if (isRetain())
          sl.emplace_back("retain");
        else if (isWeak())
          sl.emplace_back("weak");
        else if (lang != SrcLangExt::CSharp && isStrong())
          sl.emplace_back("strong");
        else if (isUnretained())
          sl.emplace_back("unsafe_unretained");

        if (!isObjCMethod())
        {
          if (protection() == Protection::Protected)
            sl.emplace_back("protected");
          else if (protection() == Protection::Private)
            sl.emplace_back("private");
          else if (protection() == Protection::Package)
            sl.emplace_back("package");

          if (lvirt == Specifier::Virtual)
            sl.emplace_back("virtual");
          else if (lvirt == Specifier::Pure)
            sl.emplace_back("pure virtual");
...
void MemberDefImpl::_writeGroupInclude(OutputList &ol, bool inGroup) const
{
  ...
  if (!nm.isEmpty())
  {
    ...
    if (isIDLorJava || forceLocalIncludes)
      ol.docify("\"");
    else
      ol.docify("<");
                                       ...
                                       bool showInline) const
{
  ...
  if (container->definitionType() == TypeGroup)
  {
    if (getClassDef())
    {
      ...
      scopedContainer = getClassDef();
    }
    else if (getNamespaceDef())
    {
      scopeName       = getNamespaceDef()->displayName();
      scopedContainer = getNamespaceDef();
    }
    else if (getFileDef())
    {
      scopeName       = getFileDef()->displayName();
      scopedContainer = getFileDef();
    }
  ...
  const ArgumentList &docArgList = m_templateMaster ? m_templateMaster->argumentList() : m_defArgList;
  ol.generateDoc(docFile(),
                 docLine(),
                 scopedContainer,
                 this,                           // memberDef
  ...
  const ArgumentList &docTemplateArgList = m_templateMaster ? m_templateMaster->templateArguments() : m_tArgList;
  ol.generateDoc(docFile(),
                 docLine(),
                 scopedContainer,
                 this,                                           // memberDef
...
void MemberDefImpl::addListReference(Definition *)
{
  ...
  QCString    pdName  = pd->definitionType() == Definition::TypeClass ? (toClassDef(pd))->displayName() : pd->name();
  QCString    sep     = getLanguageSpecificSeparator(lang, TRUE);
  QCString    memArgs;
  if (!isRelated())
  {
...
void MemberDefImpl::writeTagFile(TextStream &tagFile, bool useQualifiedName, bool showNamespaceMembers) const
{
  ...
  {
    tagFile << "\" protection=\"";
    if (m_prot == Protection::Protected)
      tagFile << "protected";
    else if (m_prot == Protection::Package)
      tagFile << "package";
    else /* Private */
      tagFile << "private";
  }
