...
struct FTVNode
{
    FTVNode(bool dir, const QCString &r, const QCString &f, const QCString &a,
        ...
        isLast(TRUE),
        isDir(dir), ref(r), file(f), anchor(a), name(n), nameAsHtml(nameAsHtml_),
...
{
    Private(bool TLI) :
        topLevelIndex(TLI) { indentNodes.resize(1); }
...
static void generateJSNavTree(const FTVNodes &nodeList)
{
  ...
  if (f.is_open())
  {
    ...
    std::stable_sort(navIndex.begin(), navIndex.end(), [](const auto &n1, const auto &n2) { return !n1.url.isEmpty() && (n2.url.isEmpty() || (n1.url < n2.url)); });
