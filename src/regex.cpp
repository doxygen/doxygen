...
class PToken
{
  public:
    ...
    explicit PToken(Kind k) :
        m_rep(static_cast<uint32_t>(k) << 16) {}

    /** Create a token for an ASCII character */
    PToken(char c) :
        m_rep((static_cast<uint32_t>(Kind::Character) << 16) |
    ...
    PToken(uint16_t from, uint16_t to) :
        m_rep(static_cast<uint32_t>(from) << 16 | to) {}
...
void Ex::Private::compile()
{
  ...
  auto getNextCharacter = [&]() -> PToken {
    ...
    if (cs == '\\') // escaped character
    {
      ...
      case 'X':
      {
        ...
        {
          int d = (cs >= 'a' && cs <= 'f') ? cs - 'a' + 10 : (cs >= 'A' && cs <= 'F') ? cs - 'A' + 10
                                                           : (cs >= '0' && cs <= '9') ? cs - '0'
                                                                                      : -1;
          if (d >= 0)
          {
  ...
  while ((c = *ps))
  {
    ...
    case '^': // beginning of line (if first character of the pattern)
      ...
      addToken(ps == start ? PToken(PToken::Kind::BeginOfLine) : PToken(c));
      break;
    case '$': // end of the line (if last character of the pattern)
      prevTokenPos = tokenPos;
      addToken(*(ps + 1) == '\0' ? PToken(PToken::Kind::EndOfLine) : PToken(c));
      break;
    case '.': // any character
      prevTokenPos = tokenPos;
...
bool Ex::Private::matchAt(size_t tokenPos, size_t tokenLen, std::string_view str, Match &match, const size_t pos, int level) const
{
  ...
  auto processSequence = [this, &tokenPos, &tokenLen, &index, &str, &matchCharClass,
    ...
    if (type == OptionalRange && tok.kind() == PToken::Kind::BeginCapture)
    {
      ...
      {
        if (data[tp].kind() == PToken::Kind::BeginCapture)
          depth++;
        else if (data[tp].kind() == PToken::Kind::EndCapture)
          depth--;
        tp++;
      }
    ...
    if (tok.kind() == PToken::Kind::Character) // 'x*' or 'x?'
    {
      ...
      while (index < len && str[index] == c_tok)
      {
        index++;
        if (type == Optional) break;
      }
      tokenPos++;
    }
    else if (tok.isCharClass()) // '[a-f0-4]*' or '[...]?' -> eat matching characters
    {
      while (index < len && matchCharClass(tokenPos, str[index]))
      {
        index++;
        if (type == Optional) break;
      }
    ...
    {
      while (index < len && isStartIdChar(str[index]))
      {
        index++;
        if (type == Optional) break;
      }
    ...
    {
      while (index < len && isIdChar(str[index]))
      {
        index++;
        if (type == Optional) break;
      }
    ...
    else if (type == OptionalRange && tok.kind() == PToken::Kind::BeginCapture)
    {
      ...
      while (tokenPos < tokenLen && data[tokenPos].kind() != PToken::Kind::EndCapture)
      {
        tokenPos++;
      }
      Match rangeMatch;
