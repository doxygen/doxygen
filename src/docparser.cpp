...
void DocParser::checkArgumentName()
{
  ...
  const ArgumentList &al   = context.memberDef->isDocsForDefinition() ? context.memberDef->argumentList() : context.memberDef->declArgumentList();
  ...
  if (!context.initialStyleStack.empty())
  {
    ...
    if (lineNr != -1)
    {
      warn_doc_error(context.fileName, tokenizer.getLineNr(),
                     ...
                     "command </{}> (Probable start '{}' at line {})",
                     tagName, fileName, lineNr);
    }
    else
    {
      warn_doc_error(context.fileName, tokenizer.getLineNr(),
...
void DocParser::handleLinkedWord(DocNodeVariant *parent, DocNodeList &children, bool ignoreAutoLinkFlag)
{
  ...
  auto ignoreWord = [](const QCString &w) -> bool {
    ...
    return std::find_if(list.begin(), list.end(),
                        [&w](const auto &ignore) { return w.startsWith(ignore); }) != list.end();
  };
  ...
  if (!context.insideHtmlLink &&
      (resolveRef(context.context, context.token->name, inSeeBlock, &compound, &member, lang, TRUE, fd, TRUE) || (!context.context.isEmpty() && // also try with global scope
...
reparsetoken:
  ...
  case TokenRetval::TK_COMMAND_BS:
    ...
    case CommandType::CMD_EMPHASIS:
    {
      ...
      if (tok.is(TokenRetval::TK_NEWPARA))
        goto handlepara;
...
static QCString extractCopyDocId(const char *data, size_t &j, size_t len)
{
  ...
  if (k < len - 1 && data[k] == '&' && data[k + 1] == '&')
    j = k + 2;
  else if (k < len && data[k] == '&')
    j = k + 1;
...
#define CHECK_FOR_COMMAND(str, action) \
  do \
    if ((i + sizeof(str) < len) && literal_at(data + i + 1, str)) \
    { \
      j = i + sizeof(str); \
      action; \
    } \
          ...
          if (it == context.copyStack.end()) // definition not parsed earlier
          {
            ...
            else
            {
              ...
              if (def->definitionType() == Definition::TypeMember)
              {
                ...
                const ArgumentList &docArgList = md->templateMaster() ? md->templateMaster()->argumentList() : md->argumentList();
                                  ...
                                  const DocOptions &options)
{
  ...
  parser->context.relPath   = (!options.linkFromIndex() && ctx) ? relativePathToRoot(ctx->getOutputFileBase()) : QCString("");
