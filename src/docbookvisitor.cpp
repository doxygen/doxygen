...
void DocbookDocVisitor::operator()(const DocInclude &inc)
{
  ...
  case DocInclude::IncWithLines:
  {
    ...
    getCodeParser(inc.extension()).parseCode(m_ci, inc.context(), inc.text(), langExt, inc.stripCodeComments(), inc.isExample(), inc.exampleFile(), fd.get());
    m_t << "</computeroutput></literallayout>";
  }
  break;
...
void DocbookDocVisitor::operator()(const DocParamSect &s)
{
  ...
  {
    if (i == ncols)
      m_t << "                        <colspec colwidth=\"4*\"/>\n";
    else
      m_t << "                        <colspec colwidth=\"1*\"/>\n";
  }
  m_t << "                        <tbody>\n";
  visitChildren(s);
  m_t << "                        </tbody>\n";
