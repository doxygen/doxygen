    ...
    if (tok==TK_COMMAND_AT || tok == TK_COMMAND_BS)
    {
      switch (Mappers::cmdMapper->map(parser()->context.token->name))
      {
        ...
        case CMD_ENDSECREFLIST:
          return;
        default:
          warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"Illegal command '%s%s' as part of a \\secreflist",
        ...
        case TK_COMMAND_BS:
          switch (Mappers::cmdMapper->map(parser()->context.token->name))
          {
            case CMD_ENDLINK:
              if (isJavaLink)
              {
                warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"'{@link...' ended with '%s%s' command",
                  TK_COMMAND_CHAR(tok),qPrint(parser()->context.token->name));
              }
              goto endlink;
            default:
              warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"Illegal command '%s%s' as part of a \\link",
                  TK_COMMAND_CHAR(tok),qPrint(parser()->context.token->name));
              break;
          }
          break;
        ...
        case TK_COMMAND_BS:
          {
            ...
            switch (Mappers::cmdMapper->map(cmdName))
            {
              case CMD_REF:
                {
                  ...
                  if (tok!=TK_WHITESPACE)
                  {
                    warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"expected whitespace after '%s%s' command",
                  ...
                  else
                  {
                    ...
                    if (tok!=TK_WORD)
                    {
                      warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"unexpected token %s as the argument of '%s%s' command",
              ...
              case CMD_LINK:
                ...
                break;
              default:
                warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"Illegal command '%s%s' found as part of a <dt> tag",
                  TK_COMMAND_CHAR(tok),qPrint(cmdName));
            }
          }
          break;
...
void DocPara::handleCite(const QCString &cmdName, const int tokIn)
{
  AUTO_TRACE();
  ...
  {
    warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"expected whitespace after '%s%s' command",
      TK_COMMAND_CHAR(tokIn),qPrint(cmdName));
    return;
  }
  ...
  {
    warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"unexpected end of comment block while parsing the "
        "argument of command '%s%s'",TK_COMMAND_CHAR(tokIn),qPrint(cmdName));
    return;
  }
  else if (tok!=TK_WORD && tok!=TK_LNKWORD)
  {
    warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"unexpected token %s as the argument of '%s%s'",
        DocTokenizer::tokToString(tok),TK_COMMAND_CHAR(tokIn),qPrint(cmdName));
    return;
  }
...
void DocPara::handleEmoji(const QCString &cmdName, const int tokIn)
{
  AUTO_TRACE();
...
void DocPara::handleDoxyConfig(const QCString &cmdName, const int tokIn)
{
  // get the argument of the cite command.
  int tok=parser()->tokenizer.lex();
  if (tok!=TK_WHITESPACE)
  ...
  if (opt)
  {
    ...
    switch (opt->kind())
    {
      ...
      case ConfigOption::O_Obsolete:
        warn(parser()->context.fileName,parser()->tokenizer.getLineNr(), "Obsolete setting for '%s%s': '%s'",
              TK_COMMAND_CHAR(tokIn),qPrint(cmdName),qPrint(parser()->context.token->name));
        break;
      case ConfigOption::O_Disabled:
        warn(parser()->context.fileName,parser()->tokenizer.getLineNr(),
              "Disabled setting (i.e. not supported in this doxygen executable) for '%s%s': '%s'",
              TK_COMMAND_CHAR(tokIn),qPrint(cmdName),qPrint(parser()->context.token->name));
        break;
  ...
  {
    warn(parser()->context.fileName,parser()->tokenizer.getLineNr(), "Unknown option for '%s%s': '%s'",
         TK_COMMAND_CHAR(tokIn),qPrint(cmdName),qPrint(parser()->context.token->name));
    children().append<DocWord>(parser(),thisVariant(),parser()->context.token->name);
  }
  parser()->tokenizer.setStatePara();
...
void DocPara::handleShowDate(const QCString &cmdName, const int tokIn)
{
  AUTO_TRACE();
  QCString fmt;
  QCString date;
  ...
  if (tok!=TK_WORD)
  {
    warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"invalid <format> argument for command '%s%s'",
      TK_COMMAND_CHAR(tokIn),qPrint(cmdName));
    parser()->tokenizer.setStatePara();
    return;
  }
  ...
  {
    warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"invalid <date_time> argument for command '%s%s'",
      TK_COMMAND_CHAR(tokIn),qPrint(cmdName));
    parser()->tokenizer.setStatePara();
    return;
  }
  ...
  if (!err.isEmpty())
  {
    warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"invalid <date_time> argument for command '%s%s': %s",
      TK_COMMAND_CHAR(tokIn),qPrint(cmdName),qPrint(err));
    parser()->tokenizer.setStatePara();
    return;
  }
  ...
  for (int i=0;i<SF_NumBits;i++)
  {
    ...
    {
      warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"'%s%s' <format> parameter '%s' has %s related markers which are not specified in the <date_time> parameter '%s'. Filling in the current value for %s instead.",
          TK_COMMAND_CHAR(tokIn),qPrint(cmdName),
...
void DocPara::handleILine(const QCString &cmdName, const int tokIn)
{
  AUTO_TRACE();
  ...
  if (tok!=TK_WORD)
  {
    warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"invalid argument for command '%s%s'",
      TK_COMMAND_CHAR(tokIn),qPrint(cmdName));
    return;
  }
  parser()->tokenizer.setStatePara();
...
void DocPara::handleIFile(const QCString &cmdName, const int tokIn)
{
  AUTO_TRACE();
  int tok=parser()->tokenizer.lex();
  if (tok!=TK_WHITESPACE)
...
void DocPara::handleRef(const QCString &cmdName,const int tokIn)
{
  AUTO_TRACE("cmdName={}",cmdName);
  QCString saveCmdName = cmdName;
  int tok=parser()->tokenizer.lex();
  ...
  {
    warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"expected whitespace after '%s%s' command",
      TK_COMMAND_CHAR(tokIn),qPrint(saveCmdName));
    return;
  }
  parser()->tokenizer.setStateRef();
  tok=parser()->tokenizer.lex(); // get the reference id
  ...
  {
    warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"unexpected token %s as the argument of '%s%s'",
        DocTokenizer::tokToString(tok),TK_COMMAND_CHAR(tokIn),qPrint(saveCmdName));
    goto endref;
  }
...
void DocPara::handleSection(const QCString &cmdName,const int tokIn)
{
  AUTO_TRACE("cmdName={}",cmdName);
  QCString saveCmdName = cmdName;
  ...
  {
    warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"unexpected end of comment block while parsing the "
        "argument of command '%s%s'", TK_COMMAND_CHAR(tokIn),qPrint(saveCmdName));
    return;
  }
  else if (tok!=TK_WORD && tok!=TK_LNKWORD)
  {
...
int DocPara::handleCommand(const QCString &cmdName, const int tok)
{
  ...
  switch (cmdId)
  {
    ...
    case CMD_JAVALINK:
      ...
      break;
    case CMD_CITE:
      handleCite(cmdName,tok);
      break;
    case CMD_EMOJI:
      handleEmoji(cmdName,tok);
      break;
    case CMD_DOXYCONFIG:
      handleDoxyConfig(cmdName,tok);
      break;
    case CMD_REF: // fall through
    case CMD_SUBPAGE:
      handleRef(cmdName,tok);
      break;
    case CMD_SECREFLIST:
      {
      ...
      break;
    case CMD_SECREFITEM:
      warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"unexpected command '%s%s'",TK_COMMAND_CHAR(tok),qPrint(parser()->context.token->name));
      break;
    case CMD_ENDSECREFLIST:
      warn_doc_error(parser()->context.fileName,parser()->tokenizer.getLineNr(),"unexpected command '%s%s'",TK_COMMAND_CHAR(tok),qPrint(parser()->context.token->name));
      break;
    ...
    case CMD_INHERITDOC:
      ...
      break;
    case CMD_SHOWDATE:
      handleShowDate(cmdName,tok);
      break;
    case CMD_ILINE:
      handleILine(cmdName,tok);
      break;
    case CMD_IFILE:
      handleIFile(cmdName,tok);
      break;
    default:
