...
DocVerbatim::DocVerbatim(DocParser *parser, DocNodeVariant *parent, const QCString &context,
    ...
    DocNode(parser, parent),
    p(std::make_unique<Private>(context, text, t, isExample, exampleFile, parser->context.relPath, lang, isBlock))
{
}
    ...
    DocCompoundNode(parser, parent), m_refType(Unknown), m_isSubPage(FALSE)
{
  ...
  else if (resolveLink(context, target, true, &compound, anchor, lang, parser->context.prefix))
  {
    bool isFile = compound ? (compound->definitionType() == Definition::TypeFile ||
                                  compound->definitionType() == Definition::TypePage
                                ? TRUE
                                : FALSE)
                           : FALSE;
    ...
    if (compound && compound->isLinkable()) // ref to compound
    {
      ...
      return;
             ...
             toFileDef(compound)->generateSourceFile()) // undocumented file that has source code we can link to
    {
...
DocCite::DocCite(DocParser *parser, DocNodeVariant *parent, const QCString &target, const QCString &, CiteInfoOption opt) :
    DocNode(parser, parent)
{
...
bool DocMscFile::parse()
{
  ...
  else
  {
    warn_doc_error(parser()->context.fileName, parser()->tokenizer.getLineNr(), "included msc file '{}' is not found "
                                                                                "in any of the paths specified via MSCFILE_DIRS!",
                   p->name);
  }
  return ok;
}
...
bool DocDiaFile::parse()
{
  ...
  else
  {
    warn_doc_error(parser()->context.fileName, parser()->tokenizer.getLineNr(), "included dia file '{}' is not found "
                                                                                "in any of the paths specified via DIAFILE_DIRS!",
                   p->name);
  }
  return ok;
}
...
bool DocPlantUmlFile::parse()
{
  ...
  else
  {
    warn_doc_error(parser()->context.fileName, parser()->tokenizer.getLineNr(), "included uml file '{}' is not found "
                                                                                "in any of the paths specified via PLANTUMLFILE_DIRS!",
                   p->name);
  }
  return ok;
}
...
DocImage::DocImage(DocParser *parser, DocNodeVariant *parent, const HtmlAttribList &attribs, const QCString &name,
    ...
    DocCompoundNode(parser, parent),
    p(std::make_unique<Private>(attribs, name, t, parser->context.relPath, url, inlineImage))
{
}
  ...
  if (tok.is(TokenRetval::TK_HTMLTAG))
  {
    ...
    if (tagId == HtmlTagType::HTML_LI && !parser()->context.token->endTag) // found <li> tag
    {
      // ok, we can go on.
             ...
             parser()->context.token->endTag) // found empty list
    {
...
endlist:
  AUTO_TRACE_EXIT("retval={}", retval.to_string());
  return (retval.is_any_of(TokenRetval::RetVal_EndList, TokenRetval::RetVal_CloseXml) || parser()->context.token->name == "list") ? Token::make_RetVal_OK() : retval;
}
...
void DocPara::handleInclude(const QCString &cmdName, DocInclude::Type t)
{
  ...
  if (tok.is(TokenRetval::TK_WORD) && parser()->context.token->name == "{")
  {
    ...
    if (contains("lineno"))
    {
      if (t == DocInclude::Include)
        t = DocInclude::IncWithLines;
      else if (t == DocInclude::Snippet)
        t = DocInclude::SnippetWithLines;
      else if (t == DocInclude::DontInclude)
...
Token DocPara::handleHtmlStartTag(const QCString &tagName, const HtmlAttribList &tagHtmlAttribs)
{
  ...
  if (parser()->context.token->emptyTag && !(tagId > HtmlTagType::XML_CmdMask) &&
      tagId != HtmlTagType::UNKNOWN && tagId != HtmlTagType::HTML_IMG && tagId != HtmlTagType::HTML_BR && tagId != HtmlTagType::HTML_HR && tagId != HtmlTagType::HTML_P && tagId != HtmlTagType::HTML_DIV && tagId != HtmlTagType::HTML_SPAN)
  {
