...
HtmlDocVisitor::HtmlDocVisitor(TextStream &t, OutputCodeList &ci,
    ...
    m_t(t),
    m_ci(ci), m_ctx(ctx), m_fileName(fn)
{
...
void HtmlDocVisitor::operator()(const DocStyleChange &s)
{
  ...
  case DocStyleChange::Bold:
    if (s.enable())
      m_t << "<b" << s.attribs().toString() << ">";
    else
      m_t << "</b>";
    break;
  case DocStyleChange::S:
    if (s.enable())
      m_t << "<s" << s.attribs().toString() << ">";
    else
      m_t << "</s>";
    break;
  case DocStyleChange::Strike:
    if (s.enable())
      m_t << "<strike" << s.attribs().toString() << ">";
    else
      m_t << "</strike>";
    break;
  case DocStyleChange::Del:
    if (s.enable())
      m_t << "<del" << s.attribs().toString() << ">";
    else
      m_t << "</del>";
    break;
  case DocStyleChange::Underline:
    if (s.enable())
      m_t << "<u" << s.attribs().toString() << ">";
    else
      m_t << "</u>";
    break;
  case DocStyleChange::Ins:
    if (s.enable())
      m_t << "<ins" << s.attribs().toString() << ">";
    else
      m_t << "</ins>";
    break;
  ...
  case DocStyleChange::Subscript:
    if (s.enable())
      m_t << "<sub" << s.attribs().toString() << ">";
    else
      m_t << "</sub>";
    break;
  case DocStyleChange::Superscript:
    if (s.enable())
      m_t << "<sup" << s.attribs().toString() << ">";
    else
      m_t << "</sup>";
    break;
  ...
  case DocStyleChange::Small:
    if (s.enable())
      m_t << "<small" << s.attribs().toString() << ">";
    else
      m_t << "</small>";
    break;
  case DocStyleChange::Cite:
    if (s.enable())
      m_t << "<cite" << s.attribs().toString() << ">";
    else
      m_t << "</cite>";
    break;
  ...
  else
  {
    ...
    auto writeFormula = [&](ImageType imgType, Visibility visibility) -> QCString {
      ...
      if (visibility == Visibility::Light)
        t << " light-mode-visible";
      else if (visibility == Visibility::Dark)
        t << " dark-mode-visible";
      t << "\" alt=\"" << filterQuotedCdataAttr(f.text()) << "\""
        << " src=\"" << f.relPath() << f.name();
...
static contexts_t getParagraphContext(const DocPara &p, bool &isFirst, bool &isLast)
{
  ...
  if (p.parent())
  {
    ...
    if (parBlock)
    {
      ...
      {
        if (isLI)
          t = contexts_t::STARTLI;
        else if (isDD)
          t = contexts_t::STARTDD;
        else if (isTD)
          t = contexts_t::STARTTD;
      }
      if (isLast)
      {
        if (isLI)
      ...
      {
        if (isLI)
          t = contexts_t::INTERLI;
        else if (isDD)
          t = contexts_t::INTERDD;
        else if (isTD)
          t = contexts_t::INTERTD;
      }
      return t;
    }
