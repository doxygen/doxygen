...
{
  static const StringUnorderedSet flowKeywords({ "break", "case", "catch", "continue", "default", "do",
                                                 "else", "finally", "for", "foreach", "for each", "goto",
                                                 "if", "return", "switch", "throw", "throws", "try",
                                                 "while", "@try", "@catch", "@finally" });
  static const StringUnorderedSet typeKeywords({ "bool", "char", "double", "float", "int", "long", "object",
  ...
  if (!incName.isEmpty())
  {
    ...
    if (fn)
    {
      ...
      auto it    = std::find_if(fn->begin(),
                                fn->end(),
                                [&fd](const auto &ifd) { return fd->isIncluded(ifd->absFilePath()); });
    ...
    case CXToken_Keyword:
      ...
      else
      {
        codifyLines(ol, fd, s, line, column,
                    cursorKind == CXCursor_PreprocessingDirective ? "preprocessor" : keywordToType(s));
      }
      break;
...
ClangTUParser::ClangTUParser(const ClangParser &, const FileDef *) :
    p(std::make_unique<Private>())
{
}
