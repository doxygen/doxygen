...
#define DOC_NODES \
  ...
  DN_SEP                      DN(DocURL) \
  DN_SEP                      DN(DocLineBreak) \
  DN_SEP DN(DocHorRuler) DN_SEP /*  5 */ DN(DocAnchor) \
  DN_SEP                                 DN(DocCite) \
  DN_SEP                                 DN(DocStyleChange) \
  DN_SEP                                 DN(DocSymbol) \
  DN_SEP DN(DocEmoji) DN_SEP /* 10 */ DN(DocWhiteSpace) \
  DN_SEP                              DN(DocSeparator) \
  DN_SEP                              DN(DocVerbatim) \
  DN_SEP                              DN(DocInclude) \
  DN_SEP DN(DocIncOperator) DN_SEP /* 15 */ DN(DocFormula) \
  DN_SEP                                    DN(DocIndexEntry) \
  DN_SEP                                    DN(DocAutoList) \
  DN_SEP                                    DN(DocAutoListItem) \
  DN_SEP DN(DocTitle) DN_SEP /* 20 */ DN(DocXRefItem) \
  DN_SEP                              DN(DocImage) \
  DN_SEP                              DN(DocDotFile) \
  DN_SEP                              DN(DocMscFile) \
  DN_SEP DN(DocDiaFile) DN_SEP /* 25 */ DN(DocVhdlFlow) \
  DN_SEP                                DN(DocLink) \
  DN_SEP                                DN(DocRef) \
  DN_SEP                                DN(DocInternalRef) \
  DN_SEP DN(DocHRef) DN_SEP /* 30 */ DN(DocHtmlHeader) \
  DN_SEP                             DN(DocHtmlDescTitle) \
  DN_SEP                             DN(DocHtmlDescList) \
  DN_SEP                             DN(DocSection) \
  DN_SEP DN(DocSecRefItem) DN_SEP /* 35 */ DN(DocSecRefList) \
  DN_SEP                                   DN(DocInternal) \
  DN_SEP                                   DN(DocParBlock) \
  DN_SEP                                   DN(DocSimpleList) \
  DN_SEP DN(DocHtmlList) DN_SEP /* 40 */ DN(DocSimpleSect) \
  DN_SEP                                 DN(DocSimpleSectSep) \
  DN_SEP                                 DN(DocParamSect) \
  DN_SEP                                 DN(DocPara) \
  DN_SEP DN(DocParamList) DN_SEP /* 45 */ DN(DocSimpleListItem) \
  DN_SEP                                  DN(DocHtmlListItem) \
  DN_SEP                                  DN(DocHtmlDescData) \
  DN_SEP                                  DN(DocHtmlCell) \
  DN_SEP DN(DocHtmlCaption) DN_SEP /* 50 */ DN(DocHtmlRow) \
  DN_SEP                                    DN(DocHtmlTable) \
  DN_SEP                                    DN(DocHtmlBlockQuote) \
  DN_SEP                                    DN(DocText) \
  DN_SEP DN(DocRoot) DN_SEP /* 55 */ DN(DocHtmlDetails) \
  DN_SEP                             DN(DocHtmlSummary) \
  DN_SEP                             DN(DocPlantUmlFile)
...
class DocNode
{
  public:
    ...
    DocNode(DocParser *parser, DocNodeVariant *parent) :
        m_parser(parser), m_parent(parent) {}
    ~DocNode() = default;
  ...
  private:
    struct Private
    {
        Private(const QCString &context_, const QCString &text_, Type type_, bool isExample_,
            ...
            context(context_),
            text(text_), type(type_), isExample(isExample_),
...
class DocInclude : public DocNode
{
  public:
    ...
    QCString extension() const
    {
      ...
      return i != -1 ? m_file.mid(i) : QCString();
    }
...
class DocAutoList : public DocCompoundNode
{
  public:
    enum ListType
    {
      ...
      Unchecked  = -2,
      Checked_x  = -3,
      Checked_X  = -4 // positive numbers give the label
    };
...
class DocDiagramFileBase : public DocCompoundNode
{
  public:
    DocDiagramFileBase(DocParser *parser, DocNodeVariant *parent, const QCString &name,
        ...
        DocCompoundNode(parser, parent),
        p(std::make_unique<Private>(name, context, srcFile, srcLine)) {}
    QCString name() const { return p->name; }
...
class DocText : public DocCompoundNode
{
  public:
    DocText(DocParser *parser) :
        DocCompoundNode(parser, nullptr) {}
    void parse();
    bool isEmpty() const { return children().empty(); }
};
