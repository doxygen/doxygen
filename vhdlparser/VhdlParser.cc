...
outlineParser()->addVhdlType(s2.data(),outlineParser()->getLine(ALIAS_T),Entry::VARIABLE_SEC,VhdlSpecifier::ALIAS,0,s.data(),Protection::Public);
...
QCString t=s1+"::"+s;
    ...
    outlineParser()->addVhdlType(t.data(),outlineParser()->getLine(ARCHITECTURE_T),Entry::CLASS_SEC,VhdlSpecifier::ARCHITECTURE,0,0,Protection::Private);
    }
    if (!hasError) {
    try {
      if (!hasError) {
...
outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(ATTRIBUTE_T),Entry::VARIABLE_SEC,VhdlSpecifier::ATTRIBUTE,0,s1.data(),Protection::Public);
...
QCString t= s1+" is "+s2;
   outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(ATTRIBUTE_T),Entry::VARIABLE_SEC,VhdlSpecifier::ATTRIBUTE,0,t.data(),Protection::Public);
...
m_sharedState->currP=VhdlSpecifier::COMPONENT;
    ...
    if (!hasError) {
outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(COMPONENT_T),Entry::VARIABLE_SEC,VhdlSpecifier::COMPONENT,0,0,Protection::Public);
...
m_sharedState->confName=s+"::"+s1;
  outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(CONFIGURATION_T),Entry::VARIABLE_SEC,VhdlSpecifier::CONFIG,"configuration",s1.data(),Protection::Public);
    }
    if (!hasError) {
...
if(t)
      s2.prepend(":=");
     QCString it=s1+s2;
     outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(CONSTANT_T),Entry::VARIABLE_SEC,VhdlSpecifier::CONSTANT,0,it.data(),Protection::Public);
     it.prepend("constant ");
     return it;
assert(false);
}
...
m_sharedState->parse_sec=0;
                          outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(LIBRARY_T),Entry::VARIABLE_SEC,VhdlSpecifier::LIBRARY,"context",s1.data(),Protection::Public);
}
...
m_sharedState->lastEntity=m_sharedState->current;
                ...
                outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(ENTITY_T),Entry::CLASS_SEC,VhdlSpecifier::ENTITY,0,0,Protection::Public);
    }
    if (!hasError) {
    entity_header();
    }
...
QCString t1=s2+" "+s3;
   outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(),Entry::VARIABLE_SEC,VhdlSpecifier::VFILE,0,t1.data(),Protection::Public);
   return " file "+s+":"+s2+" "+s3+";";
assert(false);
}
...
tmpEntry=m_sharedState->current;
    outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(),Entry::VARIABLE_SEC,VhdlSpecifier::RECORD,0,0,Protection::Public);
    }
    if (!hasError) {
    s2 = type_definition();
    }
...
outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(),Entry::VARIABLE_SEC,VhdlSpecifier::VFILE,0,s1.data(),Protection::Public);
...
QCString q;
   ...
   if (m_sharedState->parse_sec==GEN_SEC)
   {
     ...
     if (a>b) b=a;
     outlineParser()->addVhdlType(m_sharedState->current->name.data(),b,Entry::VARIABLE_SEC,VhdlSpecifier::GENERIC,ss.data(),0,Protection::Public);
   }
...
if (m_sharedState->currP!=VhdlSpecifier::COMPONENT && m_sharedState->interf_sec==false)
    {
      ...
      {
        outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(),Entry::VARIABLE_SEC,VhdlSpecifier::GENERIC,s1.data(),"",Protection::Public);
      }
...
if ( m_sharedState->parse_sec==0 && Config_getBool(SHOW_INCLUDE_FILES) )
                   {
                           outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(),Entry::VARIABLE_SEC,VhdlSpecifier::LIBRARY,s.data(),"_library_",Protection::Public);
                   }
...
m_sharedState->lastCompound=m_sharedState->current;
                        s.prepend("_");
                        outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(),Entry::CLASS_SEC,VhdlSpecifier::PACKAGE_BODY,0,0,Protection::Protected);
    }
    if (!hasError) {
...
m_sharedState->lastCompound=m_sharedState->current;
                          ...
                          outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(PACKAGE_T),Entry::CLASS_SEC,VhdlSpecifier::PACKAGE,0,0,Protection::Package);
    }
    if (!hasError) {
    package_header();
    }
...
QCString q=" is new "+s1+s2;
      outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(PACKAGE_T),Entry::VARIABLE_SEC,VhdlSpecifier::INSTANTIATION,"package",q.data(),Protection::Public);
}
...
QCString VhdlParser::physical_type_definition() {QCString s,s1,s2;Token *t=0;
    ...
    if (!hasError) {
outlineParser()->addVhdlType(s.data(),t->beginLine,Entry::VARIABLE_SEC,VhdlSpecifier::UNITS,0,0,Protection::Public);
    }
    if (!hasError) {
    while (!hasError) {
      switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
...
outlineParser()->addVhdlType(s.data(),t1->beginLine,Entry::VARIABLE_SEC,VhdlSpecifier::UNITS,0,s1.data(),Protection::Public);
   return s+"="+s1;
assert(false);
}
...
if(tok)
      s3.prepend(":=");
     s4=s1+s2+s3;
     outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(),Entry::VARIABLE_SEC,VhdlSpecifier::SIGNAL,0,s4.data(),Protection::Public);
}
...
QCString q= " is new "+s1+s2;
      outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(FUNCTION_T),Entry::VARIABLE_SEC,VhdlSpecifier::INSTANTIATION,"function ",q.data(),Protection::Public);
    return q;
assert(false);
}
...
outlineParser()->addVhdlType(s.data(),outlineParser()->getLine(),Entry::VARIABLE_SEC,VhdlSpecifier::SUBTYPE,0,s1.data(),Protection::Public);
