...
TuneColorDialog::TuneColorDialog(int hue, int sat, int gamma, QWidget *parent) :
    QDialog(parent)
{
...
void ColorPicker::paintEvent(QPaintEvent *)
{
  ...
  QPolygon a;
  int      y = m_mode == Hue ? hue2y(m_hue) : m_mode == Saturation ? sat2y(m_sat)
                                                                   : gam2y(m_gam);
...
{
  if (m_mode == Hue)
    setHue(y2hue(getMouseYPositionFromEvent(m)));
  else if (m_mode == Saturation)
    setSat(y2sat(getMouseYPositionFromEvent(m)));
  else
    setGam(y2gam(getMouseYPositionFromEvent(m)));
}
...
Step1::Step1(Wizard *wizard, const QHash<QString, Input *> &modelData) :
...
void Step2::init()
{
  m_extractModeGroup->button(
                      getBoolOption(m_modelData, STR_EXTRACT_ALL) ? 1 : 0)
    ->setChecked(true);
  ...
  if (getBoolOption(m_modelData, STR_CPP_CLI_SUPPORT))
    ...
    x = 5;
  else if (getBoolOption(m_modelData, STR_OPTIMIZE_OUTPUT_SLICE))
    x = 6;
