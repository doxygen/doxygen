...
struct WordPosition
{
    WordPosition(size_t s, size_t i) :
        start(s), index(i) {}
    size_t start;
    size_t index;
};
...
static void usage(const char *name, int exitVal = 1)
{
  ...
  std::cerr << "       "
            << "alternatively the query string can be given by the environment variable QUERY_STRING" << std::endl;
  exit(exitVal);
}
    ...
    for (std::vector<std::string>::const_iterator it = parts.begin(); it != parts.end(); ++it)
    {
      ...
      if (kv.size() == 2)
      {
        ...
        if (kv[0] == "q")
          ...
          num = fromString<int>(val);
        else if (kv[0] == "p")
          page = fromString<int>(val);
        else if (kv[0] == "cb")
          callback = val;
      }
    }
